...
class ImageFigure(TemplateFilter):
     name = "image_figure"

     def resource_file_to_path(image_file: str) -> str:
          return f'../resources/{image_file}'
     ...
     def filter(info) -> str:
          ...
          label = ""
          if len(info) == 1:
               image_file = ImageFigure.resource_file_to_path(info[0])
          elif len(info) == 2:
               image_file = ImageFigure.resource_file_to_path(info[0])
               caption = info[1]
          elif len(info) == 2:
               ...
               image_file = ImageFigure.resource_file_to_path(info)
          else:
               raise ValueError(
                    "Unknown arg set for Image Figure"
               )
          return (r"\begin{figure}[h]" + "\n" + 
                  r"  \centering" + "\n" +
                  r"  \includegraphics[width=0.25\textwidth]{"+image_file+"}\n" + 
                  (r"  \caption{"+caption+"}\n" if caption != "" else  "") + 
                  (r"  \label{"+label+"}\n" if caption != "" else  "") + 
                  r"\end{figure}" + "\n")
